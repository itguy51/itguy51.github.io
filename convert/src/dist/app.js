...
//Backend.js
//var request = require('then-request'); // Replace, use jsonp.
//var SpotifyWebApi = new SpotifyWebApi();
//var levenshtein = require('fast-levenshtein');

var spotifyApi = new SpotifyWebApi(); //Shame ur own API is gon make u get rekt.
//HEY LOOK A SPOTIFY TO APPLE MUSIC CONVERSION
var terms = [" edit ", " mix ", " - "]; // Get rid of them. Good reason - iTunes doesn't use them where Spotify does.
String.prototype.replaceBlankout = function(find) {
...
function useIDMS(IDMSBundle, callmeback) {
    IDMSBundle = decode(IDMSBundle);
    var dat = JSON.parse(IDMSBundle);
    var enc = encodes((removeDiacritics(dat.artist) + " " + removeDiacritics(dat.track).toLowerCase().replaceBlankout(terms)));
    request('GET', 'https://itunes.apple.com/search?term=' + enc).then(function(data) {
        //console.log('https://itunes.apple.com/search?term=' + data);
        return request('GET', 'https://itunes.apple.com/search?term=' + data);
    }).then(function(reg) {
        return JSON.parse(reg.getBody('utf8'));
    }).then(function(json) {
        tracksPoss = [];

        for (var i = 0; i < json.resultCount; i++) {
            var artistMatch = (removeDiacritics(json.results[i].artistName.toLowerCase().replace(/\./g, ' ')).indexOf(removeDiacritics(temp.artists[0].name.toLowerCase()))) > -1;

            //var artistMatch = (removeDiacritics(json.results[i].artistName.toLowerCase()) == removeDiacritics(temp.artists[0].name.toLowerCase()));
            //var trackMatch = (removeDiacritics(json.results[i].trackName.toLowerCase()).indexOf(removeDiacritics(temp.name.toLowerCase()))) > -1;
            var trackMatch = (removeDiacritics(json.results[i].trackName.toLowerCase()) == removeDiacritics(temp.name.toLowerCase()));
            //console.log(json.results[i].artistName + ", " + json.results[i].trackName + ", " + temp.name);
            if (artistMatch && trackMatch) {

                var num = billCosby(json.results[i], temp.artists[0].name.toLowerCase(), temp.name.toLowerCase());
                tracksPoss.push([Math.floor(num - 1), json.results[i]]); //The lower the num, the higher the conf.

            } else if (!trackMatch && artistMatch) {
                log("NTM");
                var num = billCosby(json.results[i], temp.artists[0].name.toLowerCase(), temp.name.toLowerCase());
                tracksPoss.push([num + 6, json.results[i]]);
            } else if (!artistMatch && trackMatch) {
                log("NAM");
                var num = billCosby(json.results[i], temp.artists[0].name.toLowerCase(), temp.name.toLowerCase());
                tracksPoss.push([num - 0, json.results[i]]);
            } else {
                log("NM");
                var num = billCosby(json.results[i], temp.artists[0].name.toLowerCase(), temp.name.toLowerCase());
                tracksPoss.push([num * 2, json.results[i]]);
            }
            //console.log(i);
        };
        tracksPoss = tracksPoss.sort(compareConf);
        //console.log(tracksPoss);
        callmeback(tracksPoss);
    })
}
*/
function convertTrack(spotifyURI, callmeback) {
    //wut wut
    log(spotifyURI);
    var temp;
    spotifyApi.getTrack(spotifyURI).then(function(dat) {
        log(dat)
        temp = dat;
        var enc = (removeDiacritics(dat.artists[0].name).toLowerCase() + " " + removeDiacritics(dat.name).toLowerCase().replaceBlankout(terms)); //OK WE'RE DONE WITH SPOOTIFY NOW
        log(enc)
        enc = encodes(enc)
       // enc = enc.toLowerCase();
        return enc;
        ...
        //console.log(data);
        var uc = "https://itunes.apple.com/search?callback=cb&term=" + data;
        //console.log(uc);
        //console.log('https://itunes.apple.com/search?term=' + data);
        return $.ajax({
            url: uc,
...
function encodes(thing) {
    return encodeURIComponent(thing).replace(/%20/g, "+");
}

function billCosby(zip, zop, bop) {
    //I was so tired when I named this. No judging.
    //zip{arr} zop{artist} bop{song}
    var jello;
    jello = Levenshtein.get(removeDiacritics(zip.artistName.toLowerCase()), removeDiacritics(zop)) + 1 * Levenshtein.get(removeDiacritics(zip.trackName.toLowerCase()), removeDiacritics(bop)) + 1;
    log(jello)
    return Math.round(jello);

}

function concatArtists(song) {
    var artists = [];
    artistArray = song.artists;
